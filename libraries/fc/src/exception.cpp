...
std::exception_ptr
unhandled_exception::get_inner_exception() const {
    return _inner;
}

NO_RETURN void
unhandled_exception::dynamic_rethrow_exception() const {
...
unhandled_exception::dynamic_copy_exception() const {
...
exception::name() const throw() {
    return my->_name.c_str();
}

const char*
exception::what() const throw() {
    return my->_what.c_str();
}
...
void
to_variant(const exception& e, variant& v) {
    v = mutable_variant_object("code", e.code())("name", e.name())("message", e.what())("stack", e.get_log());
}

void
from_variant(const variant& v, exception& ll) {
...
const log_messages&
exception::get_log() const {
    return my->_elog;
}

void
exception::append_log(log_message m) {
 ...
 */
string
exception::to_detail_string(log_level ll) const {
    std::stringstream ss;
    try {
        try {
 ...
 */
string
exception::to_string(log_level ll) const {
    std::stringstream ss;
    try {
        ss << my->_what;
        try {
...
void NO_RETURN
exception_factory::rethrow(const exception& e) const {
 ...
 */
NO_RETURN void
exception::dynamic_rethrow_exception() const {
...
void
throw_bad_enum_cast(int64_t i, const char* e) {
...
void
throw_bad_enum_cast(const char* k, const char* e) {
    FC_THROW_EXCEPTION(bad_cast_exception,
...
record_assert_trip(
    ...
    uint32_t    lineno,
    const char* expr) {
    fc::mutable_variant_object assert_trip_info = fc::mutable_variant_object()("source_file", filename)("source_lineno", lineno)("expr", expr);
